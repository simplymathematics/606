...
```
This looks fairly linear, but I am not yet confident that this is a good enough model.
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    There appears to be a linear relationship between the number of batting opportunities and the number of runs scored, however, there is significant variance.
...
```
The number of hits is a better predictor of the number of runs scored than the at bats. 
...
summary(m2)
```
This shows us a strong, positive relationship between homeruns and runs. 
...
5578*.6305 + -2789
```
This model underpredicts this range by about 20 runs.
## Model diagnostics
...
The residuals plot is fairly random, indicating a linear relationship between runs and at-bats.
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    ...
    While there is some systemic bias in our model, the linear model appears to be a good enough approximation.
* * *
...
```
There appears to be a linear relationship between batting average and the runs scored.
...
summary(m1)
```
The new model, m3, is a far more reliable an indicator than m1 as indicated by the larger R^2 value.
...
```
Each of these appears to have a stronger linear relationship than any of the features we examined above. In particular, the onbase+ slugs statistics is the best indicator because theu inidicate a generalized offense term (onbase) and a generalized defense one (slug). 
...
summary(m4)
```
This indicator predicts 93% of the variance by itself.
<div id="license">
