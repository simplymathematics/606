...
There are 1000 samples in the NC data, but this is presumably less than 10% of the population.
...
```
The sample size is > 30 and <10% of the population.

4.  Write the hypotheses for testing if the average weights of babies born to 
smoking and non-smoking mothers are different.

Null hypothesis: The average weights of babies born to smoking and non smoking mothers are the same.  

Alternative Hypothesis: average weights of babies born to smoking and non smoking mothers are different.
...
```
We reject the alternative hypothesis and conclude that younger and more mature moms gain the same amount of weight.
...
```
The cutoff age is 35 between mature and younger moms as we can see in the above boxplot.
...
Null Hypothesis: There is no correlation between birthweight and length of pregnancy.
Alternative Hypothesis: There is a correlation between birthweight and length of pregnancy.
```{r}
inference(y = nc$weeks, x = nc$lowbirthweight, est = "mean", type = "ci", null = 0, 
          alternative = "twosided", method = "theoretical", order = c("low", "not low"))
```
As we can see from the boxplots, the low birthweights are much more likely with shorter pregnancies and we can reject the Null Hypothesis.
