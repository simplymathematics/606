...
It appears that more girls were baptized over the course of the 17th century.
...
ratioBoys <- arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls)
```
...
```{r}
plot(arbuthnot$year, ratioBoys, type = "l")
```
...
While time periods are on the same order of magnitude, the present population is much, much higher (and the records include more people that just people baptized)
...
```{r}
ratioOfBoys <- present$boys / (present$girls+present$boys)
plot(present$year, ratioBoys)
```
...
maximum <- max(total)
row <- which(total==maximum, arr.ind=TRUE)
year <- present$yearlibrary(DATA606)
startLab('Lab0')[row]
year
```
