...
The population distrubutino has a mean between 1-2k square feet. It is also skewed right.
...
The sample has much less skew on the right side and is more heavily weighted towards smaller areas than the true distrubtion.
...
As $n$ increases the sample distribution approaches the population distribution.
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    There are 5000 sample_means, centered near 1475 sq feet.
    
    ```{r}
    length(sample_means50)
    ```
If we increase the number of samples, the sampling distribution will begin to take on a normal shape.
```{r loop, eval=TRUE}
...
```
There are 50 means of samples in ```sample_means_small.``` 
...
The mean approaches the true mean and the variance decreases.
...
```
The sample distribution mean is 180942.
...
The sampling distrubutino with a larger sample has less variance which is preferable.
