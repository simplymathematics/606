...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    These data were taken from a poll where we can only estimate population parameters.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    We must assume that the sample observations are independend, that the sample size is <10% of the population, and that the samples have been picked randomly. We also expect to see 10 successes and 10 failures. Except for the truly random sampling, these limitations are feasible. The random sampling will be hard given that there is no known list of all people and some type of convenience sampling is likely to occur.
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    Table 6 is the religious, not religious and atheist percentages per country. Each row in atheism corresponds to the response of an indivudal person.
...
```
The computed value from the atheism frame agrees with the Table 6 value of 5%.
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    We assume that observations are independent, that the poll is a sample random poll with <10% of teh population with a success-failure condition. The sample size of 1002 is large enough as a success rate of 5% is greater than 10. 
...
This function is parabolic, centered at .5 and concave down. It's asymptotes are at 0 and 1.
...
for(i in 1:5000){
  tmp <- sample(c("atheist", "non_atheist"), n, replace = TRUE, prob = c(p, 1-p))
  p_hats[i] <- sum(tmp == "atheist")/n
}
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    No. Ecuador only has 8 successes (.02 * 400).
...
-   Answer the following two questions using the `inference` function. As 
    ...
    $H_0$: there is no evidence that Spain has seen a change in its atheism from 2005-20012
    $H_a$: there is evidence that Spain has seen a change in its atheism from 2005-2012
...
n12 = 1145
PE = p12 - p05
SE = sqrt((p05*(1-p05)/n05)+(p12*(1-p12)/n12))
top <- PE + SE*1.96
bottom <- PE - SE*1.96
interval <- c(bottom, top)
interval
```

We failed to reject the null hypothesis because the confidence intervals overlap.
...
interval
```
There is evidence to reject the null hypothesis because the control interval does not contain 0. 
-   If in fact there has been no change in the atheism index in the countries 
    ...
    *Hint:* Look in the textbook index under Type 1 error.
    ...
    Those countries with large difference in measurements but a small margin of error will could lead to a rejection of the null hypothesis when in fact it is true.
