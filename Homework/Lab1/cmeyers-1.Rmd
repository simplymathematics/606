...
genhlth: a categorical vector

exerany: a categorical vector, 
	hlthplan: a categorical vector, 
	smoke100: a categorical vector,
	height: a discrete vector, 
	weight: a discrete vector, 
	wtdesire: a discrete vector, 
	age: a discrete vector, 
...
4657 people report being in excellent health

```{r}
excellent <- subset(cdc, cdc$genhlth=='excellent')
dim(excellent)
```
...
A low BMI seems to correspond with better health. I think exerany has a relationship to BMI because people who excercise tend to to weigh less.
...
```
There appears to be a strong positive correlation between weight and desired weight.
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    this mean about the person's weight and desired weight.
    What if `wdiff` is 
    positive or negative?

If wdiff = 0, then the person is as their ideal weight. If wdiff is negative, it means they want to lose weight, and positive means the opposite.
...
median(wdiff)
```
The average person wants to lose 15 pounds, but the median person only wants to lose 10. This means that the data is skweed towards people who want to lose weight, but that people are more or less happy where they are (don't want to lose a lot of weight)

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
```{r}
mdata <- subset(cdc, cdc$gender == 'male')
fdata <- subset(cdc, cdc$gender == 'female')
wdiffm <- mdata$wtdesire - mdata$weight
wdifff <- fdata$wtdesire - fdata$weight
summary(wdiffm)
summary(wdifff)
cdc <- cbind(cdc, wdiff)
...
l = mean - (sd/2)
u = mean + (sd/2)
percent = sum(cdc$weight>=l & cdc$weight <= u) / total * 100
mean
sd
total
l
